...
#define VCPU_THREAD_STACKSIZE   CONFIG_VCPU_THREAD_STACKSIZE

/**
 * @brief VM Status description.
 * VM_STATE_RESET: VM_STATE_RUNNING->VM_STATE_HALT->VM_STATE_RUNNING
 */
#define VM_STATE_NEVER_RUN      (BIT(0)) /* VM 已创建，尚未启动 */
#define VM_STATE_RUNNING        (BIT(1)) /* VM 正在执行 */
#define VM_STATE_PAUSE          (BIT(2)) /* VM 已暂停，上下文保存在内存 */
#define VM_STATE_HALT           (BIT(3)) /* VM 已停止，资源未释放 */
#define VM_STATE_ERROR          (BIT(4)) /* VM 发生错误 */
...
#define VM_VIRTIO_BLK_NAME          "VIRTIOBLK"
#define VM_DEFAULT_VIRTIO_BLK_BASE  DT_REG_ADDR(DT_CHOSEN(vm_blk))
#define VM_DEFAULT_VIRTIO_BLK_IRQ   DT_IRQN(DT_CHOSEN(vm_blk))
#endif
#if defined(CONFIG_VM_VIRTIO_MMIO) && defined(CONFIG_VM_VIRTIO_NET)
#define VM_VIRTIO_NET_NAME          "VIRTIONET"
#define VM_DEFAULT_VIRTIO_NET_BASE  DT_REG_ADDR(DT_CHOSEN(vm_net))
#define VM_DEFAULT_VIRTIO_NET_IRQ   DT_IRQN(DT_CHOSEN(vm_net))
#endif
#define VM_DEFAULT_VIRTIO_NAME_LEN  (9)
...
struct z_vcpu {
    ...
    uint16_t cpu;
    uint16_t exit_type;
    uint16_t pre_alloc_pcpu;

    /**
     * @brief hcpuipi_count is used to record the ipi
     * count from the host cpu.
     * Host may send ipi to vcpu to sync and flush irq.
     */
    atomic_t hcpuipi_count;
    atomic_t vcpuipi_count;
    atomic_t vcpu_state;
...
int vm_create(int vm_tmpid, struct z_vm *new_vm);
int vm_delete(struct z_vm *vm);

void list_all_vms_info(void);

int vm_ipi_handler(struct z_vm *vm);

uint32_t get_vm_status(uint16_t vmid);

...
struct zvm_manage_info {
    ...
    int vcpu_use[CONFIG_MP_MAX_NUM_CPUS];
...
static ALWAYS_INLINE uint32_t find_next_vmid(uint32_t *vmid)
{
...
/**
 * @brief Free a unique vmid to the ZVM.
 */
static ALWAYS_INLINE int free_vmid(uint32_t vmid)
{
    k_spinlock_key_t key;

    /* Check if the VMID is valid */
    if (vmid >= CONFIG_MAX_VM_NUM) {
        return -EINVAL;  /* Invalid VMID */
    }

    key = k_spin_lock(&zvm_overall_info->spin_zmi);

    /* Check if the VMID is allocated */
    if (!(zvm_overall_info->alloced_vmid & BIT(vmid))) {
        k_spin_unlock(&zvm_overall_info->spin_zmi, key);
        return -ENOENT;  /* VMID not allocated */
    }

    /* Clear the allocated bit for the given VMID */
    zvm_overall_info->alloced_vmid &= ~BIT(vmid);

    /* Decrease the total VM count */
    zvm_overall_info->vm_total_num--;

    k_spin_unlock(&zvm_overall_info->spin_zmi, key);

    return 0;  /* Successfully freed the VMID */
}

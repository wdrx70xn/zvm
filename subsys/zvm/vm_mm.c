 ...
 */
#include <autoconf.h> /* 避免提示不允许使用不完整的类型 */
            ...
            uint64_t ipbase, uint64_t size, uint32_t attrs)
{
    ...
    memset(vpart, 0, sizeof(struct vm_mem_partition));
    if (!vpart) {
        return NULL;
    }
    vpart->vm_mm_partition = (struct k_mem_partition *)
            k_malloc(sizeof(struct k_mem_partition));
    memset(vpart->vm_mm_partition, 0, sizeof(struct k_mem_partition));
...
static int vm_dtb_mem_create(struct vm_mem_domain *vmem_domain)
{
    int ret = 0;
    ARG_UNUSED(ret);
    uint32_t vm_dtb_size = vmem_domain->vm->os->info.linux_dtb_load_size;
    uint64_t vm_dtb_base = vmem_domain->vm->os->info.linux_dtb_load_addr;
...
static int vm_mem_domain_partition_remove(struct vm_mem_domain *vmem_dm)
{
    ...
    arch_vm_mem_table_count_clean(domain, 4 * vm_max_partitions, vm->vmid);
#endif
    k_free(domain);
    k_spin_unlock(&vm_mem_domain_lock,key);
...
int vm_mem_apart_remove(struct vm_mem_domain *vmem_dm)
{
    ...
    free_guest_memory_part(vm->os->info.vm_load_base);
    k_spin_unlock(&vmem_dm->spin_mmlock,key);
    return ret;
}
...
int vm_mem_domain_create(struct z_vm *vm)
{
    ...
    memset(vmem_dm, 0, sizeof(struct vm_mem_domain));
    if (!vmem_dm) {
    ...
    memset(vmem_dm->vm_mm_domain, 0, sizeof(struct k_mem_domain));
...
uint64_t vm_gpa_to_hpa_setvpart(struct z_vm *vm, uint64_t gpa, struct vm_mem_partition **out_vpart)
{
    struct vm_mem_domain *vmem_domain = vm->vmem_domain;
    sys_dnode_t *d_node, *ds_node;
    uint64_t vpart_gpa_start, vpart_gpa_end, vpart_hpa_start;
...
#define MAX_VIRTIO_TYPE     3

// 内存映射结构体，用于记录某个 VM 当前的映射状态
struct host_mem_map {
    uint8_t *hva;             // 主机虚拟地址（host virtual address）
    uint64_t mapped_hpa;      // 当前映射的主机物理地址起始（host physical address）
    size_t mapped_size;       // 当前映射的长度（单位：字节）
    struct k_spinlock lock;   // 自旋锁保护映射状态
    bool valid;               // 当前映射是否有效
    int vmid;                 // 所属虚拟机编号
};

// 多个 VM 的读写映射表（按 VM ID 索引）
static struct host_mem_map read_maps[CONFIG_MAX_VM_NUM][MAX_VIRTIO_TYPE] = { 0 };
static struct host_mem_map write_maps[CONFIG_MAX_VM_NUM][MAX_VIRTIO_TYPE] = { 0 };

/**
 * @brief 从主机物理内存读取数据（支持 VM 分离映射）
 *
 * @param hpa   主机物理地址
 * @param dst   目标内存地址（拷贝目的地）
 * @param len   要读取的字节数
 * @param vmid  虚拟机编号，用于选择对应映射槽
 * @param type  VirtIO设备类型，目前blk和net设备不能用同一个map，需要做区分
 */
void vm_host_memory_read(uint64_t hpa, void *dst, size_t len, int vmid, int type)
{
    if (!dst || len == 0 || vmid < 0 || vmid >= CONFIG_MAX_VM_NUM ||
        type < 0 || type >= MAX_VIRTIO_TYPE) {
        return;
    }

    struct host_mem_map *map = &read_maps[vmid][type];
    k_spinlock_key_t key = k_spin_lock(&map->lock);

    const size_t page_size = CONFIG_MMU_PAGE_SIZE;
    const uint64_t aligned_hpa = hpa & ~(page_size - 1);
    const size_t aligned_len = ROUND_UP(hpa + len - aligned_hpa, page_size);

    bool need_remap = !map->valid ||
                      (hpa < map->mapped_hpa) ||
                      ((hpa + len) > (map->mapped_hpa + map->mapped_size));

    if (need_remap) {
        if (map->hva) {
            k_mem_unmap_phys_bare(map->hva, map->mapped_size);
        }

        uint8_t *new_hva = NULL;
        k_mem_map_phys_bare(&new_hva, aligned_hpa, aligned_len,
                            K_MEM_CACHE_WB | K_MEM_PERM_RW);

        map->hva = new_hva;
        map->mapped_hpa = aligned_hpa;
        map->mapped_size = aligned_len;
        map->valid = (new_hva != NULL);

        if (map->valid) {
            arch_dcache_invd_range((void *)map->hva, aligned_len);
        }
    }

    if (map->valid) {
        const uintptr_t offset = hpa - map->mapped_hpa;
        barrier_dmem_fence_full();
        memcpy(dst, map->hva + offset, len);
        barrier_dmem_fence_full();
    } else {
        memset(dst, 0, len);
        ZVM_LOG_WARN("[VM %d][type %d] Failed to map and read from HPA: 0x%llx\n", vmid, type, hpa);
    }
...
/**
 * @brief 向主机物理内存写入数据（支持 VM 分离映射）
 *
 * @param hpa   主机物理地址
 * @param src   源内存地址（数据来源）
 * @param len   要写入的字节数
 * @param vmid  虚拟机编号，用于选择对应映射槽
 * @param type  VirtIO设备类型，目前blk和net设备不能用同一个map，需要做区分
 */
void vm_host_memory_write(uint64_t hpa, void *src, size_t len, int vmid, int type)
{
    if (!src || len == 0 || vmid < 0 || vmid >= CONFIG_MAX_VM_NUM ||
        type < 0 || type >= MAX_VIRTIO_TYPE) {
        return;
    }

    struct host_mem_map *map = &write_maps[vmid][type];
    k_spinlock_key_t key = k_spin_lock(&map->lock);
    ...
    if (map->valid) {
        const uintptr_t offset = hpa - map->mapped_hpa;
        barrier_dmem_fence_full();
        memcpy(map->hva + offset, src, len);
        barrier_dmem_fence_full();
        arch_dcache_invd_range((void *)(map->hva + offset), len);
    } else {
        ZVM_LOG_WARN("[VM %d][type %d] Failed to map and write to HPA: 0x%llx\n", vmid, type, hpa);
    }
...
void vm_guest_memory_read(struct z_vm *vm, uint64_t gpa, void *dst, size_t len, int type)
{
    uint64_t hpa;
    struct vm_mem_partition *vpart = NULL;
    // vpart = (struct vm_mem_partition *)k_malloc(sizeof(struct vm_mem_partition));
    // memset(vpart, 0, sizeof(struct vm_mem_partition));
    // if (!vpart) {
    //     return;
    ...
    vm_host_memory_read(hpa, dst, len, vm->vmid, type);
}

void vm_guest_memory_write(struct z_vm *vm, uint64_t gpa, void *src, size_t len, int type)
{
    uint64_t hpa;
    struct vm_mem_partition *vpart = NULL;
    // vpart = (struct vm_mem_partition *)k_malloc(sizeof(struct vm_mem_partition));
    ...
    vm_host_memory_write(hpa, src, len, vm->vmid, type);
}

...
static int intra_vm_msg_handler(struct z_vm *vm)
{
    ...
    set_virq_to_vcpu(vcpu, vcpu->virq_block.pending_sgi_num, 1);
    return 0;
}
...
static void list_vm_info(uint16_t vmid)
{
    ...
    printk(
        "|*** %-6d%-14s%-7d%-10d%-9s***|\n",
        vm->vmid, vm->vm_name, vm->vcpu_num, mem_size, vm_ss);

}

uint32_t get_vm_status(uint16_t vmid) {
    return zvm_overall_info->vms[vmid]->vm_status;
}
...
int vm_create(int vm_tmpid, struct z_vm *new_vm)
{
    ...
    memset(vm->os, 0, sizeof(struct z_os));
	...
	ret = vm_os_create(vm->os, vm_tmpid);
    if (ret) {
        k_free(vm->os);
    ...
    ret = vm_vcpus_create(vm->os->info.vcpu_num, vm);
    if (ret) {
    ...
    memset(vm->arch, 0, sizeof(struct vm_arch));
    ...
    memset(vm->ops, 0, sizeof(struct zvm_ops));
    if (!vm->ops) {
...
int vm_vcpus_create(uint16_t vcpu_num, struct z_vm *vm)
{
    ...
    memset(vm->vcpus, 0,vcpu_num * sizeof(struct z_vcpu *));
    if (!(vm->vcpus)) {
...
int vm_vcpus_init(struct z_vm *vm)
{
    ...
    for (i = 0; i < vm->vcpu_num; i++) {
        ...
        // ret = k_sem_init(&vm->vcpu_exit_sem[i], 0, 1);
        // if(ret < 0){
        //     ZVM_LOG_ERR("sem init error\n");
        // }
    ...
    atomic_set(&vm->exist_vcpu, vm->vcpu_num);
    print_vcpu_usage();
    return ret;
}
...
{
    uint16_t i=0;
    int ret = 0, vm_status_switch_faild = 0;
    struct z_vcpu *vcpu;
    ...
    for(i = 0; i < vm->vcpu_num; i++){
        ...
        if(vm->vm_status == VM_STATE_PAUSE) {
            ret = vm_vcpu_ready(vcpu);
        }
        if (!vcpu->is_poweroff) {
            ret = vm_vcpu_ready(vcpu);
        }
        if (ret < 0) {
            vm_status_switch_faild = 1;
            ZVM_LOG_WARN("vm_vcpu_ready failed, vcpu_id: %d\n", vcpu->vcpu_id);
        }
    }
    if (!vm_status_switch_faild) {
...
int vm_vcpus_halt(struct z_vm *vm)
{
    ...
    for(i = 0; i < vm->vcpu_num; i++){
        ...
        ret = vm_vcpu_halt(vcpu);
        if (ret < 0) {
            vm_status_switch_faild = 1;
            ZVM_LOG_WARN("vm_vcpu_halt failed, vcpu_id: %d\n", vcpu->vcpu_id);
        } else {
            ZVM_LOG_INFO("halt vcpu%d successfully\n",i);
        }
    }
    if (!vm_status_switch_faild) {
...
int vm_delete(struct z_vm *vm)
{
    ...
    for(int i = 0; i < vm->vcpu_num; i++){
        ...
        ret = vm_vcpu_deinit(vcpu);
        if (ret < 0) {
            vm_status_switch_faild = 1;
        }

#ifndef CONFIG_SOC_QEMU_MAX
        ZVM_LOG_INFO("Ready to clean VM's PCPU %d ...\n", pcpu_id);
        set_pcpu_cache_clean(pcpu_id);
    ...
    ret = free_vmid(vm->vmid);
    if (ret) {
        ZVM_LOG_WARN("Free vmid failed! \n");
        return ret;
    }
    return ret;
}

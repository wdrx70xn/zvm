...
static int vpl011_regs_init(struct z_vm *vm, struct virt_pl011 *pl011, uint32_t serial_base, uint32_t serial_size)
{
...
static int vserial_vdev_mem_read(struct z_virt_dev *vdev, uint64_t addr, uint64_t *value, uint16_t size)
{
    uint32_t offset;
	uint32_t *v;
	...
	*value = 0;
	...
	// printk("vserial_vdev_mem_read%llx\n",offset);
	if (offset == 0) {
		...
		struct virt_serial *serial = vs->vserial;

        if (vs->count != 0) {
			k_mutex_lock(&serial->send_buffer.buffer_mutex, K_MSEC(30));
        	*v = serial->send_buffer.data[serial->send_buffer.read_pos];
			atomic_set(&serial->send_buffer.read_pos, (serial->send_buffer.read_pos + 1) % SEND_BUFFER_SIZE);
			atomic_dec(&serial->send_buffer.count);
			vs->count--;
        	if (serial->send_buffer.count == 0) {
        	    serial->send_buffer.read_pos = 0;
        	    serial->send_buffer.write_pos = 0;
        	}
			k_mutex_unlock(&serial->send_buffer.buffer_mutex);
        }
		if (vs->count>0)
		{
			VDEV_REGS(vdev)->ris |= VPL011_INT_RX;
...
static int vserial_vdev_mem_write(struct z_virt_dev *vdev, uint64_t addr, uint64_t *value, uint16_t size)
{
    uint32_t offset;
    struct virt_pl011 *vs = (struct virt_pl011 *)vdev->priv_vdev;
...
static int pl011_virt_serial_send(struct virt_serial *serial, unsigned char *data,int len)
{
	...
	// 检查缓冲区空间
    if (serial->send_buffer.write_pos + len > SEND_BUFFER_SIZE) {
        serial->send_buffer.buffer_overflow = true;
		ZVM_LOG_ERR("vserial send_buffer overflow!\n");
        return -ENOSPC;
    }

	vpl011 = (struct virt_pl011 *)get_virt_serial_device(serial);

    k_mutex_lock(&serial->send_buffer.buffer_mutex, K_MSEC(30));
    // 写入数据到缓冲区
    for (size_t i = 0; i < len; i++) {
        serial->send_buffer.data[serial->send_buffer.write_pos] = data[i];
        // serial->send_buffer.write_pos = (serial->send_buffer.write_pos + 1) % SEND_BUFFER_SIZE;
		atomic_set(&serial->send_buffer.write_pos, (serial->send_buffer.write_pos + 1) % SEND_BUFFER_SIZE);
        // serial->send_buffer.count++;
		atomic_inc(&serial->send_buffer.count);
    }
	k_mutex_unlock(&serial->send_buffer.buffer_mutex);

	vpl011->count = serial->send_buffer.count;
...
static int vm_virt_serial_init(const struct device *dev, struct z_vm *vm, struct z_virt_dev *vdev_desc)
{
	...
	/* Init virtual vpl011 device for virtual device. */
	vpl011 = (struct virt_pl011 *)k_malloc(sizeof(struct virt_pl011));
	memset(vpl011, 0, sizeof(struct virt_pl011));
    if (!vpl011) {
        ZVM_LOG_ERR("Allocat memory for vserial error \n");
        return -ENODEV;
    }
	...
	strncat(name, "/vpl011", sizeof(name));

    vpl011->vserial = virt_serial_create(name, &pl011_virt_serial_send, vpl011);

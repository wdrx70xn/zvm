...
{
    struct virt_timer_context *timer_ctxt;

    timer_ctxt = vcpu->arch->vtimer_context;
#ifdef CONFIG_HAS_ARM_VHE

    /* virt timer save */
    timer_ctxt->cntv_ctl = read_cntv_ctl_el02();
    timer_ctxt->cntv_cval = read_cntv_cval_el02();
    timer_ctxt->timer_offset = read_cntvoff_el2();
    write_cntv_ctl_el02(0);
#else
...
static void vcpu_vtimer_load(struct z_vcpu *vcpu)
{
    ...
    if(vcpu->is_first_run) {
        vcpu->is_first_run = false;
        if (vcpu->vcpu_id != 0) {
            struct virt_timer_context *maincore_ctxt;
            /* For vcpu, we need to set the vtimer offset value */
            maincore_ctxt = vcpu->vm->vcpus[0]->arch->vtimer_context;
            timer_ctxt->timer_offset = maincore_ctxt->timer_offset;
        } else {
            timer_ctxt->cntp_cnt = arm_arch_timer_count();
            timer_ctxt->timer_offset = read_cntvct_el0();
         }
    }
    write_cntvoff_el2(timer_ctxt->timer_offset);
    write_cntv_cval_el02(timer_ctxt->cntv_cval);
    write_cntv_ctl_el02(timer_ctxt->cntv_ctl);
...
static void arch_vcpu_sys_regs_init(struct z_vcpu *vcpu)
{
    ...
    memset(aarch64_c->sys_regs, 0, sizeof(aarch64_c->sys_regs));

    aarch64_c->sys_regs[VCPU_MIDR_EL1] = read_midr_el1();
    ...
    aarch64_c->sys_regs[VCPU_MPIDR_EL1] = aarch64_c->sys_regs[VCPU_MPIDR_EL1] & BIT64_MASK(20);
#endif
#if defined(CONFIG_SOC_RK3568) || defined(CONFIG_SOC_RK3588)
    aarch64_c->sys_regs[VCPU_MPIDR_EL1] = aarch64_c->sys_regs[VCPU_MPIDR_EL1] & BIT64_MASK(12);
#endif
#if defined(CONFIG_SOC_QEMU_MAX)
    aarch64_c->sys_regs[VCPU_MPIDR_EL1] = aarch64_c->sys_regs[VCPU_MPIDR_EL1] & BIT64_MASK(4);
#endif
...
ALWAYS_INLINE uint64_t*  find_index_reg(uint16_t index, arch_commom_regs_t *regs)
{
    uint64_t *value;
...
void vcpu_sysreg_load(struct z_vcpu *vcpu)
{
    ...
    write_vpidr_el2(g_context->sys_regs[VCPU_MIDR_EL1]);
    write_vmpidr_el2(g_context->sys_regs[VCPU_MPIDR_EL1]);
    write_actlr_el1(g_context->sys_regs[VCPU_ACTLR_EL1]);
...
void vcpu_sysreg_save(struct z_vcpu *vcpu)
{
    ...
    g_context->sys_regs[VCPU_MIDR_EL1] = read_vpidr_el2();
    ...
    g_context->sys_regs[VCPU_TPIDR_EL0] = read_tpidr_el0();
    ...
    g_context->sys_regs[VCPU_ELR_EL1] = read_elr_el12();
    g_context->sys_regs[VCPU_SPSR_EL1] = read_spsr_el12();
...
void switch_to_guest_sysreg(struct z_vcpu *vcpu)
{
    ...
    __asm__ volatile(
        ...
        "isb\n"
        ::
        [vtcr]        "r" (vcpu->vm->arch->vtcr_el2),
        [vttbr]       "r" (vcpu->vm->arch->vttbr)
        : "memory"
    );
    ...
    write_tpidr_el1(gcontext->sys_regs[VCPU_TPIDR_EL1]);
    write_vmpidr_el2(gcontext->sys_regs[VCPU_MPIDR_EL1]);
...
void switch_to_host_sysreg(struct z_vcpu *vcpu)
{
    ...
    gcontext->sys_regs[VCPU_TPIDR_EL1] = read_tpidr_el1();
...
void ALWAYS_INLINE arch_vcpu_context_save(struct z_vcpu *vcpu)
{
    vcpu_vgic_save(vcpu);
...
void ALWAYS_INLINE arch_vcpu_context_load(struct z_vcpu *vcpu)
{
...
int arch_vcpu_init(struct z_vcpu *vcpu)
{
    ...
    vcpu_arch->ctxt.regs.esf_handle_regs.x0 = vcpu->vm->os->info.linux_dtb_load_addr;
    vcpu_arch->ctxt.regs.esf_handle_regs.x1 = 0;
    vcpu_arch->ctxt.regs.esf_handle_regs.x2 = 0;
    vcpu_arch->ctxt.regs.esf_handle_regs.x3 = 0;
    vcpu_arch->ctxt.regs.callee_saved_regs.x20 = vcpu->vm->os->info.linux_dtb_load_addr;

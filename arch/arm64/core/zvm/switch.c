...
#ifdef CONFIG_SOC_FAMILY_ROCKCHIP
            ...
            struct esr_dabt_area *dabt, arch_commom_regs_t *regs)
{
    ...
    if (!iss_isv) {
#ifdef CONFIG_SOC_FAMILY_ROCKCHIP
        if ((pa_addr >= RKLD_START_PHY_ADDR) && (addr < RKLD_START_PHY_ADDR + RKLD_SIZE))
        {
            return 0;
        }
#endif
        ZVM_LOG_WARN("Instruction syndrome not valid %x\n", iss_isv);
        return -EFAULT;
    }
...
static ALWAYS_INLINE int cpu_unknwn_sync(arch_commom_regs_t *arch_ctxt, uint64_t esr_elx)
{
...
static int cpu_wfi_wfe_sync(arch_commom_regs_t *arch_ctxt, uint64_t esr_elx)
{
    ...
    /* WFE */
    if(esr_iss & 0x01){
        if(atomic_get(&vcpu->vcpu_state) == _VCPU_STATE_RUNNING){
            vm_vcpu_ready(vcpu);
        }
    }else{  /* WFI */
...
static ALWAYS_INLINE int cpu_dmcr_mrc_sync(arch_commom_regs_t *arch_ctxt, uint64_t esr_elx)
{
    ARG_UNUSED(arch_ctxt);
    ARG_UNUSED(esr_elx);
	return 0;
...
static ALWAYS_INLINE int cpu_dmcrr_mrrc_sync(arch_commom_regs_t *arch_ctxt, uint64_t esr_elx)
{
    ARG_UNUSED(arch_ctxt);
    ARG_UNUSED(esr_elx);
	return 0;
...
static ALWAYS_INLINE int cpu_simd_fp_sync(arch_commom_regs_t *arch_ctxt, uint64_t esr_elx)
{
    ARG_UNUSED(arch_ctxt);
    ARG_UNUSED(esr_elx);
	return 0;
...
static ALWAYS_INLINE int cpu_il_exe_sync(arch_commom_regs_t *arch_ctxt, uint64_t esr_elx)
{
    ARG_UNUSED(arch_ctxt);
    ARG_UNUSED(esr_elx);
	return 0;
...
static ALWAYS_INLINE int cpu_hvc64_sync(struct z_vcpu *vcpu, arch_commom_regs_t *arch_ctxt, uint64_t esr_elx)
{
    int ret = 0;
...
static int cpu_system_msr_mrs_sync(arch_commom_regs_t *arch_ctxt, uint64_t esr_elx)
{
    ...
    uint64_t *reg_value;
    struct esr_sysreg_area *esr_sysreg;
    struct z_vcpu *vcpu = _current_vcpu;

    esr_sysreg = (struct esr_sysreg_area *)&this_esr;
	...
	case ESR_SYSINSREG_ASGI1R_EL1:
		if (!esr_sysreg->dire) {
            vgicv3_raise_sgi2vcpu(vcpu, *reg_value);
        }
		break;
...
static ALWAYS_INLINE int cpu_inst_abort_low_sync(arch_commom_regs_t *arch_ctxt, uint64_t esr_elx)
{
    uint64_t ipa_ddr;
...
static ALWAYS_INLINE int cpu_inst_abort_cur_sync(arch_commom_regs_t *arch_ctxt, uint64_t esr_elx)
{
    ARG_UNUSED(arch_ctxt);
    ARG_UNUSED(esr_elx);
	return 0;
...
static ALWAYS_INLINE int cpu_misaligned_pc_sync(arch_commom_regs_t *arch_ctxt, uint64_t esr_elx)
{
    ARG_UNUSED(arch_ctxt);
    ARG_UNUSED(esr_elx);
	return 0;
...
static ALWAYS_INLINE int cpu_data_abort_cur_sync(arch_commom_regs_t *arch_ctxt, uint64_t esr_elx)
{
    ARG_UNUSED(arch_ctxt);
    ARG_UNUSED(esr_elx);
	return 0;
...
static ALWAYS_INLINE int cpu_misaligned_sp_sync(arch_commom_regs_t *arch_ctxt, uint64_t esr_elx)
{
    ARG_UNUSED(arch_ctxt);
    ARG_UNUSED(esr_elx);
	return 0;
...
handler_failed:
    ZVM_LOG_WARN("ZVM do not support this exit code: %llx. \n", GET_ESR_EC(esr_elx));
    return -ENODEV;
}

static ALWAYS_INLINE void vm_disable_daif(void)
{
...
static ALWAYS_INLINE void vm_enable_daif(void)
{
...
static ALWAYS_INLINE int arch_vm_irq_trap(struct z_vcpu *vcpu)
{
    ARG_UNUSED(vcpu);
    vm_enable_daif();
    return 0;
...
static ALWAYS_INLINE void arch_vm_serror_trap(struct z_vcpu *vcpu, int exit_code)
{
    uint64_t disr;
    uint64_t esr;
...
#ifdef CONFIG_VCPU_MIGRATING_INFO
static int8_t vcpu_last_cpu[CONFIG_MAX_VM_NUM][CONFIG_MAX_VCPU_PER_VM] = {
    [0 ... CONFIG_MAX_VM_NUM-1][0 ... CONFIG_MAX_VCPU_PER_VM-1] = -1
};
#endif
int arch_vcpu_run(struct z_vcpu *vcpu)
{
    ...
    uint16_t current_vmid = vcpu->vm->vmid;
    uint16_t vcpu_id = vcpu->vcpu_id;
    uint8_t current_cpuid = arch_curr_cpu()->id;

    if (vcpu_last_cpu[current_vmid][vcpu_id] != current_cpuid) {
        if (vcpu_last_cpu[current_vmid][vcpu_id] == -1) {
            printk("%s vCPU %d first runs on CPU %d\n",
                   vcpu->vm->vm_name, vcpu_id, current_cpuid);
        } else {
            printk("%s vCPU %d migrated from CPU %d -> CPU %d\n",
                   vcpu->vm->vm_name, vcpu_id,
                   vcpu_last_cpu[current_vmid][vcpu_id], current_cpuid);
        }
        vcpu_last_cpu[current_vmid][vcpu_id] = current_cpuid;
    }
#endif
	...
	case ARM_VM_EXCEPTION_SYNC:
        ...
        if (ret < 0) {
            ZVM_LOG_ERR("Trap sync failed, ret: %d\n", ret);
            return ret;
        }
        break;
    ...
    /*time slice condition meet, but no */
    if(read_cntp_ctl_el0() == (CNTP_CTL_ENABLE_BIT | CNTP_CTL_ISTAT_BIT | CNTP_CTL_IMASK_BIT)) {
        arm_gic_irq_set_pending(ARM_TIMER_HYP_IRQ);
    }
...
bool zvm_switch_handle_pre(uint32_t irq)
{
    ...
    if(!vcpu->vm->vm_irq_block.pt_irq_bitmap[irq]){
        return false;
    }
...
ALWAYS_INLINE uint64_t get_zvm_host_context(void)
{
...
ALWAYS_INLINE void* z_vm_asm_get_vcpu(void)
{
    struct z_vcpu *vcpu = _current_vcpu;
    if (vcpu == NULL) {
	...
	vcpu->arch->fault.esr_el2 = read_esr_el2();
    return (void*)vcpu;
}
